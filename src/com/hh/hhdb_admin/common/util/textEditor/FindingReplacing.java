...
import com.hh.frame.swingui.view.textEditor.rSyntaxTextArea.ui.rtextarea.SearchResult;
...
public class FindingReplacing {
    public HDialog dlog;
    private QueryEditorTextArea qed;
     ...
     * @param qed
     */
    public FindingReplacing(QueryEditorTextArea qed) {
        this.qed = qed;
    ...
    public void show(String find) {
        try {
            ...
            if (null == dlog) {
                dlog = new HDialog(StartUtil.parentFrame, 600, 280,false){
                    @Override
    ...
    private void cancel(){
        try {
            ...
            sea.setSearchFor("");
            SearchEngine.markAll(qed.getTextArea(), sea);
            sumLabel.setValue("");
    ...
    private void init() {
        ...
        panel.add(new WithLabelInput(new HPanel(new HDivLayout(GridSplitEnum.C3)), " ", null),
                new WithLabelInput(new HPanel(new HDivLayout(GridSplitEnum.C3)), " ", null),
                cInput,sumLabel);
	...
	private void operation(String type) {
        ...
        try {
            ...
            if (type.equals("replace") || type.equals("all")) {
                ...
                if (type.equals("all")){
                    sr = SearchEngine.replaceAll(qed.getTextArea(), context);
                    JOptionPane.showMessageDialog(null, sr.wasFound() ? "替换成功，替换数量："+sr.getCount() : "没有匹配内容！");
                }else {
                    sr = SearchEngine.replace(qed.getTextArea(), context);
                    if (!sr.wasFound() && cInput.getValue().contains("loop")) {
                        //初始化光标位置
                        qed.getTextArea().setCaretPosition(0);
                        sr = SearchEngine.replace(qed.getTextArea(), context);
                    }
                ...
                sr = SearchEngine.find(qed.getTextArea(), context);
        
                if (!sr.wasFound() && cInput.getValue().contains("loop")) {
                    //初始化光标位置
                    qed.getTextArea().setCaretPosition(type.equals("last") ? Integer.max(qed.getText().length(),0) : 0);
                    sr = SearchEngine.find(qed.getTextArea(), context);
                }
