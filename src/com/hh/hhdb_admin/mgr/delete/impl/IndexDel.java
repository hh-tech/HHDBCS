...
public class IndexDel extends AbsDel {
	...
	public void del(NodeInfo nodeInfo) throws Exception {
		...
		try {
			List<Map<String, String>> primaryKeyLists = isPrimaryKey(conn, schemaName, tableName, name);
			if (primaryKeyLists.size() > 0) {
				Constraint constraint = new Constraint(dbType, conn);
				List<String> list = constraint.getDelSql(TreeMrType.PRIMARY_KEY.name(), schemaName, tableName,
	...
	public List<Map<String, String>> isPrimaryKey(Connection conn, String schema, String tableName, String indexName)
			throws SQLException {
		String sql;
		switch (dbType) {
		case oracle:
			sql = "SELECT ac.constraint_name  as \"constraint_name\",\n" + "       ac.constraint_type  AS \"type\",\n"
					...
					+ "WHERE ac.OWNER = '%s'\n" + "  and ac.table_name = '%s'" + "  and ac.constraint_type = 'P'"
		...
		case mysql:
			sql = "SELECT CONSTRAINT_SCHEMA,CONSTRAINT_NAME ,TABLE_SCHEMA,TABLE_NAME,CONSTRAINT_TYPE "
					...
					+ "AND TABLE_NAME='%s' " + "AND CONSTRAINT_TYPE = 'PRIMARY KEY' " + "AND CONSTRAINT_NAME = '%s'";
			break;
		case pgsql:
		case hhdb:
			HHdbPgsqlPrefixEnum prefix = DBTypeEnum.hhdb.equals(dbType) ? HHdbPgsqlPrefixEnum.hh
					: HHdbPgsqlPrefixEnum.pg;
			schema = HHdbObjSqlUtil.getSchemaId(conn, schema, prefix);
			sql = "select cons.conname as constraint_name " + "from  " + prefix + "_constraint cons " + "inner join "
					+ prefix + "_class  c on cons.conrelid = c.oid " + "where c.relnamespace='%s' "
					+ "and c.relname = '%s' " + "and cons.contype='p' " + "and cons.conname = '%s'";
			break;
		case sqlserver:
			...
			if (versionBean.bigVer < 8) {
				sql = "select b.name constraint_name from sysobjects a,sysobjects b "
						+ "LEFT JOIN sys.schemas s ON s.schema_id=b.schema_id " + "where a.ID=b.parent_object_id ";
				if (StringUtils.isNotBlank(schema)) {
					sql += "AND s.name='%s' ";
				} else {
					sql += "AND s.name='dbo' ";
				}
				sql += " and a.name='%s' " + " and b.type = 'Pk' " + " and b.name = '%s'";
			} else {
				sql = "select b.name constraint_name\n" + "  from sys.sysobjects a, sys.objects b\n"
						...
						+ "  and a.name = '%s'\n" + "  and b.type = 'Pk' " + "  and b.name = '%s'";
			}
			break;
		case db2:
			sql = "select CONSTNAME constraint_name from SYSCAT.TABCONST " + "where TABSCHEMA = '%s' "
					+ "and TABNAME = '%s' " + "AND TYPE = 'PK' " + "AND CONSTNAME = '%s'";
			break;
		default:
			throw new IllegalStateException("Unexpected value: " + dbType);
		}
