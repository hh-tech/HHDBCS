...
public class UserDel extends AbsDel {
	...
	public void del(NodeInfo nodeInfo) throws Exception {
		...
		case pgsql:
			...
			break;
		case oracle:
			execute(String.format("DROP USER %s CASCADE", realName));
			break;
		case mysql:
			execute(String.format("DROP USER %s", UsrMrUtil.toMysqlUsrHost(name)));
			break;
		case sqlserver:
			...
			try {
				...
				for (String db : privileges) {
					String dbUser = String.format(
							"use %s;SELECT DP.name \n" + "FROM sys.database_principals DP ,sys.server_principals SP\n"
									+ "WHERE SP.sid = DP.sid AND SP.name = '%s'",
							db, name);
				...
				batchExecute(Arrays.asList(buffer.toString().split(System.lineSeparator())));
			} finally {
				conn.close();
			}
			break;
