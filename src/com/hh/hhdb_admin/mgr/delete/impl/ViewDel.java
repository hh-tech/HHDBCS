 ...
 * @Date: 2021/9/9 12:19
 */
public class ViewDel extends AbsDel {
    ...
    public void del(NodeInfo nodeInfo) throws Exception {
        ...
        switch (dbType) {
            ...
            case dm:
                ...
                break;
            case mysql:
                sql = String.format("DROP VIEW `%s`.`%s`", schemaName, nodeName);
                break;
            case hhdb:
            case pgsql:
            case db2:
                sql = String.format("DROP VIEW %s.%s CASCADE", SqlStrUtil.dealDoubleQuote(dbType, schemaName),
                        SqlStrUtil.dealDoubleQuote(dbType, nodeName));
                break;
            case sqlserver:
                sql = String.format("DROP VIEW %s.%s", SqlStrUtil.dealDoubleQuote(dbType, schemaName),
                        SqlStrUtil.dealDoubleQuote(dbType, nodeName));
                break;
            default:
