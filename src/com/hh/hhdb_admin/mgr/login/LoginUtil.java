...
public class LoginUtil {
	...
	private static void setSchema(String dbType, JdbcBean jdbcBean, String isAdmin) {
		switch (DBTypeEnum.valueOf(dbType)) {
			...
			case db2:
				if (StringUtils.isEmpty(jdbcBean.getSchema())) {
					jdbcBean.setSchema(TreeMrUtil.getDb2ServerName(jdbcBean));
				}
				break;
			case sqlserver:
				if (StringUtils.isEmpty(jdbcBean.getSchema())) {
	...
	private static void dealMysqlConn(JdbcBean jdbc) {
		...
		if (StringUtils.isEmpty(urlSchema) && StringUtils.isEmpty(schema)) {
			...
			if (StringUtils.isNotEmpty(urlSchema) && StringUtils.isEmpty(schema)) {
				if (!urlSchema.startsWith(code) && !urlSchema.endsWith(code)) {
					urlSchema = String.format("%s%s%s", code, urlSchema, code);
				}
				jdbc.setSchema(urlSchema);
			} else {
				int n = dbUrl.indexOf("?");
				if (n != -1) {
					String prefix = dbUrl.substring(0, n);
					String endFix = dbUrl.substring(n);
					n = prefix.indexOf("/", 13);
					if (n != -1) {
						prefix = prefix.substring(0, n);
					}
					dbUrl = prefix + "/" + schema + endFix;
				} else {
					if (dbUrl.endsWith("/")) {
						dbUrl = dbUrl + schema;
					} else {
				...
				if (!schema.startsWith(code) && !schema.endsWith(code)) {
					schema = String.format("%s%s%s", code, schema, code);
				}
