...
public class EncodingDetect {
	 ...
	 * @return
	 */
	public static Set<String> getUsedCode() {
		return new LinkedHashSet<>(Arrays.asList(UTF_8, GB_2312, GBK, GB_18030));
	}
	...
	public static String getFileEncode(byte[] rawtext) {
		BytesEncodingDetect s = new BytesEncodingDetect();
		int encoding = 6;
		if (rawtext != null && rawtext.length > 0) {
			encoding=s.detectEncoding(rawtext);
		}
		return BytesEncodingDetect.javaname[encoding];
	}

	public static void main(String[] args) {
		long start = System.currentTimeMillis();
		System.out.println(getFileEncode("E:\\Downloads\\win7_64_zhitu_3.0_Setup.exe"));
...
//			long n = file.length() > 1024 * 10 ? 1024 * 10 : file.length();
//			rawtext = new byte[((int) n)];
			try (FileInputStream stream = new FileInputStream(file)) {
				byte[] rawtext = toByteArray(stream);
			...
			for (i = 0; i < rawtextlen; i++) {
				...
				if ((rawtext[i] & (byte) 0x7F) == rawtext[i]) { // One byte
					...
					if (-64 <= rawtext[i] && rawtext[i] <= -33 && i + 1 < rawtextlen && rawtext[i + 1] <= -65) {
						goodbytes += 2;
						i++;
					} else if (-32 <= rawtext[i] && rawtext[i] <= -17 && i + 2 < rawtextlen && rawtext[i + 1] <= -65 && -128 <= rawtext[i + 2] && rawtext[i + 2] <= -65) {
						goodbytes += 3;
						i += 2;
					}
			}
