 ...
 * 
 */
public class SessionStatsSeries extends JPanel implements Runnable {
	private static final long serialVersionUID = 1L;
	private TimeSeries activeSeries = new TimeSeries("活跃");
	private TimeSeries idleSeries = new TimeSeries("空闲");
	private TimeSeries totalSeries = new TimeSeries("总共");
	private List<TimeData> list = new ArrayList<TimeData>();
	private Connection hc;
	private ScheduledExecutorService executor;
	private ServerBean serverbean;

	public SessionStatsSeries(ScheduledExecutorService executor,
		...
		JFreeChart chart = ChartFactory.createTimeSeriesChart("数据库会话", "", "",
	...
	public void run() {
		...
		String sql = "select "
				+ "(SELECT count(*) FROM "+StartUtil.prefix+"_stat_activity WHERE datname = '"
				+ serverbean.getDBName()
				+ "') as \"total\","
				+ "(SELECT count(*) FROM "+StartUtil.prefix+"_stat_activity WHERE state = 'active' AND datname = '"
				+ serverbean.getDBName()
				+ "') as \"active\","
				+ "(SELECT count(*) FROM "+StartUtil.prefix+"_stat_activity WHERE state = 'idle' AND datname = '"
				+ serverbean.getDBName() + "') as \"idle\";";
		...
		try {
			...
			for (TimeData d : list) {
				activeSeries
						.addOrUpdate(
								d.getSecond(),
								Integer.parseInt(d.getValue().get("active")
										.toString()));
				idleSeries.addOrUpdate(d.getSecond(),
						Integer.parseInt(d.getValue().get("idle").toString()));
				totalSeries.addOrUpdate(d.getSecond(),
						Integer.parseInt(d.getValue().get("total").toString()));
			}
	...
	private static void configFont(JFreeChart chart) {
		...
		if (r instanceof XYLineAndShapeRenderer) {
			...
			renderer.setLegendItemToolTipGenerator(new StandardXYSeriesLabelGenerator(
					"Tooltip {0}"));// 鼠标移到序列线上提示信息为“Toolop + 序列线的名字”
		}
