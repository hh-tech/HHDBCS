 ...
 * 
 */
public class TiStatsSeries extends JPanel implements Runnable {
	private static final long serialVersionUID = 1L;
	private TimeSeries insertSeries = new TimeSeries("插入");
	private TimeSeries updatesSeries = new TimeSeries("修改");
	private TimeSeries deletesSeries = new TimeSeries("删除");
	private List<TimeData> list = new ArrayList<TimeData>();
	private Connection hc;
	private ScheduledExecutorService executor;
	private ServerBean serverbean;

	public TiStatsSeries(ScheduledExecutorService executor, DBMonitor dbMonitorp) {
		...
		dataset.addSeries(updatesSeries);
		dataset.addSeries(deletesSeries);
		JFreeChart chart = ChartFactory.createTimeSeriesChart("写入元组", "", "",
	...
	public void run() {
		...
		String sql = "SELECT "
				+ "(SELECT sum(tup_inserted) FROM "+StartUtil.prefix+"_stat_database WHERE datname ='"
				+ serverbean.getDBName()
				+ "') AS \"inserts\","
				+ "(SELECT sum(tup_updated) FROM "+StartUtil.prefix+"_stat_database WHERE datname ='"
				+ serverbean.getDBName()
				+ "') AS \"updates\","
				+ "(SELECT sum(tup_deleted) FROM "+StartUtil.prefix+"_stat_database WHERE datname ='"
				+ serverbean.getDBName() + "') AS \"deletes\"; ";
		...
		try {
			...
			deletesSeries.clear();
			for (TimeData d : list) {
				insertSeries.addOrUpdate(d.getSecond(), Integer.parseInt(d
						.getValue().get("inserts").toString()));
				updatesSeries.addOrUpdate(d.getSecond(), Integer.parseInt(d
						.getValue().get("updates").toString()));
				deletesSeries.addOrUpdate(d.getSecond(), Integer.parseInt(d
						.getValue().get("deletes").toString()));
			}
