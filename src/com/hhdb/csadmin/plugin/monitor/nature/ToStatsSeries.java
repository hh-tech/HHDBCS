 ...
 * 
 */
public class ToStatsSeries extends JPanel implements Runnable {
	private static final long serialVersionUID = 1L;
	private TimeSeries fetchedSeries = new TimeSeries("获取");
	private TimeSeries returnedSeries = new TimeSeries("返回");
	private List<TimeData> list = new ArrayList<TimeData>();
	private Connection hc;
	private ScheduledExecutorService executor;
	private ServerBean serverbean;

	public ToStatsSeries(ScheduledExecutorService executor, DBMonitor dbMonitorp) {
		...
		dataset.addSeries(fetchedSeries);
		dataset.addSeries(returnedSeries);
		JFreeChart chart = ChartFactory.createTimeSeriesChart("读取元组", "", "",
	...
	public void run() {
		...
		String sql = "SELECT "
				+ "(SELECT sum(tup_fetched) FROM "+StartUtil.prefix+"_stat_database WHERE datname = '"
				+ serverbean.getDBName()
				+ "') AS \"fetched\","
				+ "(SELECT sum(tup_returned) FROM "+StartUtil.prefix+"_stat_database WHERE datname = '"
				+ serverbean.getDBName() + "') AS \"returned\"; ";
		...
		try {
			...
			fetchedSeries.clear();
			returnedSeries.clear();
			for (TimeData d : list) {
				fetchedSeries.addOrUpdate(d.getSecond(), Long.parseLong(d
						.getValue().get("fetched").toString()));
				returnedSeries.addOrUpdate(
						d.getSecond(),
						Long.parseLong(d.getValue().get("returned")
								.toString()));
			}
