 ...
 * database session
 */
public class TpsStatsSeries extends JPanel implements Runnable {
	private static final long serialVersionUID = 1L;
	private TimeSeries commitsSeries = new TimeSeries("提交数");
	private TimeSeries rollbacksSeries = new TimeSeries("回滚数");
	private TimeSeries transactionsSeries = new TimeSeries("事物数");
	private List<TimeData> list = new ArrayList<TimeData>();
	private Connection hc;
	private ScheduledExecutorService executor;
	private ServerBean serverbean;
	public TpsStatsSeries(ScheduledExecutorService executor, DBMonitor dbMonitorp) {
		...
		dataset.addSeries(commitsSeries);
		dataset.addSeries(rollbacksSeries);
		dataset.addSeries(transactionsSeries);
		JFreeChart chart = ChartFactory.createTimeSeriesChart("每秒事物数", "", "", dataset, true, true, false);
		configFont(chart);
		ChartPanel jp = new ChartPanel(chart);
		add(jp, new GridBagConstraints(0, 1, 1, 1, 1.0, 1.0, GridBagConstraints.NORTHWEST, GridBagConstraints.BOTH,
	...
	public void run() {
		...
		String sql = "SELECT " + "(SELECT sum(xact_commit) + sum(xact_rollback) FROM "+StartUtil.prefix+"_stat_database WHERE datname ='"
				+ serverbean.getDBName() + "') AS \"transactions\","
				+ "(SELECT sum(xact_commit) FROM "+StartUtil.prefix+"_stat_database WHERE datname ='" + serverbean.getDBName()
				+ "') AS \"commits\" ," + "(SELECT sum(xact_rollback) FROM "+StartUtil.prefix+"_stat_database WHERE datname ='"
				+ serverbean.getDBName() + "') \"rollbacks\"; ";

		try {
			
			List<Map<String,Object>> dblist = SqlQueryUtil.select(hc, sql);
			list.add(new TimeData(new Second(second, minute),dblist.get(0)));
			if (list.size() >= 16) {
				list = list.subList(list.size() - 16, list.size());
			}
			commitsSeries.clear();
			rollbacksSeries.clear();
			transactionsSeries.clear();

			for (TimeData d : list) {
				commitsSeries.addOrUpdate(d.getSecond(), Integer.parseInt(d.getValue().get("commits").toString()));
				rollbacksSeries.addOrUpdate(d.getSecond(), Integer.parseInt(d.getValue().get("rollbacks").toString()));
				transactionsSeries.addOrUpdate(d.getSecond(), Integer.parseInt(d.getValue().get("transactions").toString()));
			}
		} catch (Exception ee) {
	...
	private static void configFont(JFreeChart chart) {
		...
		if (r instanceof XYLineAndShapeRenderer) {
			...
			renderer.setLegendItemToolTipGenerator(new StandardXYSeriesLabelGenerator("Tooltip {0}"));// 鼠标移到序列线上提示信息为“Toolop
																										// +
																										// 序列线的名字”
		}
