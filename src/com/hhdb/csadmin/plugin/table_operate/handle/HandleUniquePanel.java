...
public class HandleUniquePanel extends JPanel implements TableModelListener, CreateTableSQLSyntax {
	...
	public String editUniqueSql() {
		...
		if (dels.size() > 0) {
			for (String st : dels) {
				sqlBuffer.append(NEW_LINE).append(ALTER_TABLE).append("\""+tabp.getSchemaName()+"\".\""+tabp.getTableName()+"\"").append(DROP_CONSTRAINT).append(st).append(SEMI_COLON);
			}
		}
		for (int i = 0; i < rows; i++) {
			if (baseTable.getValueAt(i, 2) != null) {   //修改
				...
				if (!bean.getUnqiueName().equals(baseTable.getValueAt(i, 0).toString().trim())
					...
					str.append(DROP_CONSTRAINT).append(bean.getUnqiueName()).append(COMMA);
				...
				if (baseTable.getValueAt(i, 0) != null || "".equals(baseTable.getValueAt(i, 0))) {
					...
					if (!"".equals(CommonsHelper.nullOfStr(baseTable.getValueAt(i, 3)))) {
						...
						comments.append(" ON ").append("\""+tabp.getSchemaName()+"\".\""+tabp.getTableName()+"\"").append(" IS ").append("'" + baseTable.getValueAt(i, 3) + "'").append(SEMI_COLON);
					}
				}
			}
		}
