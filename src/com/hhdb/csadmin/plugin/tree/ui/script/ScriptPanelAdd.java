...
public class ScriptPanelAdd extends JPanel {
	...
	public ScriptPanelAdd(final String scripttype,final MarScriptPanel mp){
		...
		JLabel labletit = new JLabel("增加脚本页面");
		...
		savBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					ScriptService.addScript(name, text, scripttype);
					JOptionPane.showMessageDialog(null, "添加成功！", "消息", JOptionPane.INFORMATION_MESSAGE);
					mp.refMarScript();
				} catch (SQLException e1) {
					LM.error(LM.Model.CS.name(), e1);
					JOptionPane.showMessageDialog(null, "添加失败："+e1.getMessage(), "错误", JOptionPane.ERROR_MESSAGE);
				}
			}
		});
	}
